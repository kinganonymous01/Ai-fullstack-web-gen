# AI Website Builder — Complete & Corrected Specification v2
# All 7 errors fixed. All per-file pseudocode present. Models re-added as data-access layer.
...
← Same content. Backend uses declare global for Express augmentation.
← Frontend mirrors all shared types (no declare global block).
...
export async function startContainer(files: FileItem[]): Promise<string>
  ...
  if (webcontainerInstance !== null && previewUrl === null):
    ...
    stale.teardown().catch(console.error)
...
export async function restart(files: FileItem[]): Promise<string>
  ...
  ← Stale files removed so renamed files don't create duplicates in Vite
...
export function cleanup(): void
  ...
  if (instance):     instance.teardown().catch(console.error)
```
...
try:
  ...
  OUTER LOOP (while isRunning && timeRemaining() > 0):
    ...
    commandRunner: sandbox.commands.run(cmd, {onStdout, onStderr, timeoutMs:120000})
...
BUILD PIPELINE:
  ...
  E2B SDK v1.x:
    ...
    sandbox.commands.run(cmd, {onStdout, onStderr, timeoutMs})  ← not sandbox.process.start
